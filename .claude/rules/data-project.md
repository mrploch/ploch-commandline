...
- The project **must** reference the corresponding Model project, `Ploch.{Product}.Model` (see [`project-naming.md`](./project-naming.md) — singular `Model` is mandatory).
...
- Mark the class as `public` — configurations are referenced directly by provider projects and
  unit tests, so they are part of the Data project's supported surface. This is a deliberate
  workspace decision; do not "correct" it to `internal` on an analyser's or reviewer's advice.
