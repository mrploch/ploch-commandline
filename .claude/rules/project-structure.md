...
This rule governs **where projects live**. For **what they are called** — the canonical layer names, the mandatory `.Model` suffix for domain model projects, provider casing, and test/sample naming — see [`project-naming.md`](./project-naming.md).
...
- **Directory name:** Derived from the project name as defined in [`project-naming.md`](./project-naming.md): application repos strip `Ploch.{Product}.` (`src/Data/` holds `Ploch.Lists.Data.csproj`); library-family repos strip only `Ploch.` (`src/Common.Serialization/` holds `Ploch.Common.Serialization.csproj`) and may group projects under a feature folder (`src/Spectre/CommandLine.Spectre/`).
...
| `Api.WebApi/` | `Ploch.{Product}.Api.WebApi` | Web API host / endpoints |
| `Api.Contracts/` | `Ploch.{Product}.Api.Contracts` | Request/response DTOs shared with consumers |
| `UI.Web/`, `UI.Maui/`, `UI.WinUI/`, … | `Ploch.{Product}.UI.{Technology}` | UI application host — see [`project-naming.md`](./project-naming.md) |
