...
- [ ] Directory follows [Four Names, One String](#four-names-one-string): application repos strip `Ploch.{Product}.`, library-family repos strip only `Ploch.`; a multi-product `{Area}` is an extra directory level; test directories carry their test-kind suffix; everything sits under `src/` or `tests/`.
