...
### Phase 9: Codex & Gemini Review

Both reviews are **mandatory** for every non-trivial change. Run them (they can run in parallel) and pass **full context** to each: the issue number + title + requirements, the design decisions taken and why, the diff (`git diff <base-branch>...HEAD`), verification evidence (build/test results), and a request for a structured verdict (`APPROVED` / `APPROVED_WITH_NOTES` / `CHANGES_REQUESTED` / `REJECTED` with concrete findings).
...
3. **Review all feedback** — evaluate each suggestion from both reviewers on merit.
4. **Address valid feedback** — if code changes are needed, make them and loop back to **Phase 6** (Build), then re-run the affected reviewer on the revised diff.
5. **Document disagreements** — if you disagree with a suggestion, note your reasoning (in the PR description's Design Decisions section if user-visible). This is acceptable — not every suggestion must be implemented.
