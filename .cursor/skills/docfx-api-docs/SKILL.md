...
Remove-Item api/*.yml -Force -ErrorAction SilentlyContinue   # toc.yml included - see below
Remove-Item api/.manifest -Force -ErrorAction SilentlyContinue
```

`api/toc.yml` is generated by the metadata pass and lists every API type, so it must be deleted with
the rest: keeping it keeps table-of-contents entries for types that no longer exist. Exclude it only
in a repository that hand-authors `api/toc.yml` and tracks it in git.
